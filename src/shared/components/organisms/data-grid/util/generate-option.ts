...
const generateOption = <TData>({
  ...
  return checkSort<TData>({ option, columns, stateTable, setStateTable, isSort });
};
const checkSort = <TData>({
  option,
  columns,
  stateTable,
  setStateTable,
  isSort,
}: Pick<DataGridProps<TData>, 'columns'> &
  Pick<Props<TData>, 'stateTable' | 'setStateTable'> & {
    readonly option: TableOptions<TData>;
    readonly isSort: boolean;
  }) => {
  const isSorter =
  ...
  if (isSorter && option.state) {
    ...
    if (!isSort) option.getSortedRowModel = getSortedRowModel();
  }
  return option;
};
