  ...
  const fnUpload = async (files: FileList | null) => {
    if (files) {
      for (const file of files) {
        ...
        stateEntryUpload.listFiles = !isMultiple
          ? [dataFile]
          : [...stateEntryUpload.listFiles, dataFile];
        setStateEntryUpload({ isLoading: true, listFiles: [...stateEntryUpload.listFiles] });

        fnFormatData({ file, dataFile });
      }
    }
  ...
  const fnFormatData = async ({ file, dataFile }: { file: File; dataFile: TFile }) => {
    ...
    if (data) {
      ...
      const listFiles = isMultiple
        ? stateEntryUpload.listFiles.map(item => {
        ...
        : [{ ...data, status: 'done' }];
      setStateEntryUpload({ isLoading: false, listFiles });
      handleChange?.(listFiles);
    } else {
      setStateEntryUpload({
        isLoading: false,
